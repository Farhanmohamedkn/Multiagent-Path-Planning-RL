...
class SharedGameEnv:
    ...
    def init_observations_for_multiprocessing(self):
        self.init_observation   = [self.env._observe(agent_id) for agent_id in range(1,self.num_agents+1)]
        
        

        self.init_validActions  = [self.env._listNextValidActions(agent_id) for agent_id in range(1,self.num_agents+1)]
...
def worker(agent_id,shared_env,episode_count,lock,barrier):
        ...
        # episode_count=0
        barrier.wait()
        while episode_count.value<7:
            print("New Episode started=",episode_count.value)
            # if agent_id==1:
            ...
            random_index = torch.randint(0, (len(validActions)-2), (1,)).item()
            ...
            while (episode_step_count<2):
                ...
                with lock:
                        ...
                        # # print("current agent is ",agent_id,"cur position is",cur_pos)
                        # if agent_id==1:
                        #       print("agent",agent_id,"now at",shared_env.get_cur_pos_of_agent(agent_id))
                        #       print("agent",agent_id+1,"now at",shared_env.get_cur_pos_of_agent(agent_id+1))
                        # else:
                        #     print("agent",agent_id-1,"now at",shared_env.get_cur_pos_of_agent(agent_id-1))
                        #     print("agent",agent_id,"now at",shared_env.get_cur_pos_of_agent(agent_id))
                ...
                print("agent",agent_id,"increased step counts=",episode_step_count)
                barrier.wait() #wait for all agent to inrease the steps taken
            with lock:
                episode_count.value+=1
                print("agent",agent_id,"episode_count",episode_count.value)
            if episode_count.value==num_agents:
                    shared_env.reset_env()
                    print("env reseted")
